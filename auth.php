    ...
    if ($action == 'register') {
        ...
        if($check_result->num_rows > 0)
        {
            while($row = $check_result->fetch_assoc())
            {
                ...
                {
                    echo "<script>window.alert('Your email has already been used')</script>";
                }
                else{
            ...
            {
                $write_conn->rollback();
                echo "<script>window.alert('Cannot execute insert to user')</script>";
            }
        }
    }
